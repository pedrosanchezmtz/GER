...
cra.RECEIPT_NUMBER,
 (SELECT COUNT(ara.APPLIED_CUSTOMER_TRX_ID)
		...
		rct.ATTRIBUTE11 EXPORTACION,
		   ...
		   NVL(NULL,0)AS RETAINED_TAX_BASE,
		   NVL(NULL,'002') AS RETAINED_TAX,
		   NVL(NULL,0) as RETAINED_RATE_FEE,
		   nvl(NULL,'Tasa') AS RETAINED_FACTOR_TYPE,
		  NVL(NULL,0) AS RETAINED_AMOUNT,
		  
		   TO_CHAR(ara.AMOUNT_APPLIED,'fm9999999990.00')-TO_CHAR((TO_CHAR(ara.AMOUNT_APPLIED,'fm9999999990.00')/((SELECT MAX(rctl.TAX_RATE) FROM RA_CUSTOMER_TRX_LINES_ALL rctl WHERE 1 = 1 AND rctl.CUSTOMER_TRX_ID = rct.CUSTOMER_TRX_ID  group by CUSTOMER_TRX_ID) + 100)) *(SELECT MAX(rctl.TAX_RATE) FROM RA_CUSTOMER_TRX_LINES_ALL rctl WHERE 1 = 1 AND rctl.CUSTOMER_TRX_ID = rct.CUSTOMER_TRX_ID
		   group by CUSTOMER_TRX_ID) ,'fm9999999990.00') as  TRANSFERREDS_TAX_BASE,
		   
		   
		   NVL(NULL,'002') AS TRANSFERREDS_TAX,
		(SELECT MAX(rctl.TAX_RATE) FROM RA_CUSTOMER_TRX_LINES_ALL rctl WHERE 1 = 1 AND rctl.CUSTOMER_TRX_ID = rct.CUSTOMER_TRX_ID
		   group by CUSTOMER_TRX_ID
		   )/100 as TRANSFERREDS_RATE_FEE,
		   nvl(NULL,'Tasa') AS TRANSFERREDS_FACTOR_TYPE,
		   TO_CHAR(ara.AMOUNT_APPLIED,'fm9999999990.00')-(TO_CHAR(ara.AMOUNT_APPLIED,'fm9999999990.00')-TO_CHAR((TO_CHAR(ara.AMOUNT_APPLIED,'fm9999999990.00')/((SELECT MAX(rctl.TAX_RATE) FROM RA_CUSTOMER_TRX_LINES_ALL rctl WHERE 1 = 1 AND rctl.CUSTOMER_TRX_ID = rct.CUSTOMER_TRX_ID group by CUSTOMER_TRX_ID) + 100)) *(SELECT MAX(rctl.TAX_RATE) FROM RA_CUSTOMER_TRX_LINES_ALL rctl WHERE 1 = 1 AND rctl.CUSTOMER_TRX_ID = rct.CUSTOMER_TRX_ID group by CUSTOMER_TRX_ID) ,'fm9999999990.00')) 
		   AS TRANSFERREDS_AMOUNT
