   ...
   CREATE SEQUENCE  "XXER_BANCOS_AMEX_SEQ"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 50 START WITH 1 CACHE 20 NOORDER  NOCYCLE  NOKEEP  NOSCALE  GLOBAL ;

   CREATE SEQUENCE  "TMS_CONCILIATION_BANCOS_SEQ"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 50 START WITH 1 CACHE 20 NOORDER  NOCYCLE  NOKEEP  NOSCALE  GLOBAL ;

   CREATE SEQUENCE  "XXER_AR_CONCILIACION_S"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 NOCACHE  NOORDER  NOCYCLE  NOKEEP  NOSCALE  GLOBAL ;
...
CREATE TABLE XXER_BANCOS_AMEX_SUMMARY_TBL
   ...
   (	"CONCILIACION_ID" NUMBER(18,0) DEFAULT "XXER_AR_CONCILIACION_S"."NEXTVAL" NOT NULL ENABLE, 
	...
	COMMENT ON COLUMN "XXER_AR_CONCILIACION_TBL"."CONCILIACION_ID" IS 'id de la transaccion ';
   COMMENT ON COLUMN "XXER_AR_CONCILIACION_TBL"."REFERENCE_PAYMENT" IS 'Referencia generada  por la vista';
   COMMENT ON COLUMN "XXER_AR_CONCILIACION_TBL"."PAYMENT_NUMBER" IS 'Referencia obtenida en ERP';
   COMMENT ON COLUMN "XXER_AR_CONCILIACION_TBL"."AMOUNT_TRANSACTIONS" IS 'Sumatoria de transaciones relacionadas';
   COMMENT ON COLUMN "XXER_AR_CONCILIACION_TBL"."AMOUNT_PAYMENT" IS 'Monto de pagos consultados en ERP';
   COMMENT ON COLUMN "XXER_AR_CONCILIACION_TBL"."TOLERANCE" IS 'Diferencia de tolareancia';
   COMMENT ON COLUMN "XXER_AR_CONCILIACION_TBL"."STATUS" IS 'estatus de conciliacion corto';
   COMMENT ON COLUMN "XXER_AR_CONCILIACION_TBL"."MSJ_VALIDATION" IS 'descripcion y msj de la conciliacion';
   COMMENT ON COLUMN "XXER_AR_CONCILIACION_TBL"."REFERENCE_TRANSACTIONS_ID" IS 'id de transaciones relacionados ';
   COMMENT ON COLUMN "XXER_AR_CONCILIACION_TBL"."REFERENCE_TRANSACTIONS" IS 'numeros de documentos y montos de transacion relacionadas al pago.';
   COMMENT ON COLUMN "XXER_AR_CONCILIACION_TBL"."FECHA_CONCILIACION" IS 'Fecha que se realizo la conciliacion.';
   COMMENT ON COLUMN "XXER_AR_CONCILIACION_TBL"."METODO_RECIBO" IS 'Nombre del metodo de recibo con que se concilio el Pago.';
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "PXERINTUSER"."XXER_AR_INVOICES_CONCILIACION_V" ("NUMERO_DOCUMENTO", "TOTAL", "TABLA_PUENTE_ID", "RECEIPT_METHOD_ID", "ORGANIZATION_ID", "RFC_ORIGEN", "FRECUENCIA", "METODO_RECIBO", "REFERENCE_PAYMENT", "FECHA_MOVIMIENTO", "UNIDAD_NEGOCIO_ORIGEN", "TOLERANCIA", "FECHA_PAGO") AS 
  WITH
TXN AS (SELECT H.NUMERO_DOCUMENTO , H.FECHA_MOVIMIENTO FECHA_MOVIMIENTO , H.NOMBRE_FISCAL_ORIGEN , H.SISTEMA_ORIGEN , H.UNIDAD_NEGOCIO_ORIGEN ,
                ...
                FROM XXER_TBL_PUENTE_HEADER H, XXER_TBL_PUENTE_LINES L WHERE H.TABLA_PUENTE_ID = L.TABLA_PUENTE_ID AND H.SISTEMA_ORIGEN NOT IN ('EAM', 'INV', 'PORTAL')
                                 ...
                                 --AND TRUNC(H.FECHA_MOVIMIENTO)>= ADD_MONTHS(SYSDATE,-1)
                                 )
,
CNF AS (select NVL(REGEXP_SUBSTR(cf3.METODO_RECIBO,'[^,]+'),cf3.METODO_RECIBO) METODO_RECIBO_M,
...
FROM (SELECT
    ...
    ,(CASE
        ...
        ELSE
        (case
        WHEN CNF.FRECUENCIA_CONC_M = 'D' THEN TO_CHAR(TXN.FECHA_MOVIMIENTO,'DD/MM/RRRR')
        WHEN CNF.FRECUENCIA_CONC_M = 'S' THEN TO_CHAR(TXN.FECHA_MOVIMIENTO+(6-MOD(TO_CHAR(TXN.FECHA_MOVIMIENTO,'J'),7)),'DD/MM/RRRR')
        ELSE NULL END)
        end) as FECHA_PAGO 
...
-------------------------------------------------Vista agrupamiento de prefijos-------------------------------------------

  CREATE OR REPLACE FORCE EDITIONABLE VIEW "PXERINTUSER"."XXER_AR_CONC_ID_V" ("ID_INTERFAZ", "PREFIJO_CONC", "SUBFIJO_CONC") AS 
  WITH cf AS (
    SELECT
        id_interfaz,
        prefijo_conc,
        subfijo_conc
    FROM
        xxer_cnf_interfase_ingresos_ar
    WHERE
            1 = 1
        AND conciliacion IS NOT NULL
        AND prefijo_conc IS NOT NULL
)
SELECT
    id_interfaz,
    regexp_substr(prefijo_conc, '[^,]+', 1, LEVEL) prefijo_conc,
    subfijo_conc
FROM
    cf
CONNECT BY instr(prefijo_conc, ',', 1, LEVEL - 1) > 0
GROUP BY
    regexp_substr(prefijo_conc, '[^,]+', 1, LEVEL),
    id_interfaz,
    subfijo_conc
UNION ALL
SELECT
...
WITH
    ...
    WHERE
        trunc(ve.corte_fecha_creacion) BETWEEN to_date(v_FechaInicio, 'DD/MM/YY') AND to_date(v_FechaFin, 'DD/MM/YY')
        AND ve.tipo_pago NOT IN ( 'EFE', 'AMX' )
    ...
    FROM
        ...
        INNER JOIN tms_boletos_venta_tbl@pcentral_link.estrellaroja.com.mx          vt ON ( vt.folio_preimpreso = vm.no_folio
                                                                                   AND trunc(vt.fecha_creacion) BETWEEN to_date(v_FechaInicio,
                                                                                   'DD/MM/YY') AND to_date(v_FechaFin, 'DD/MM/YY') )
...
GROUP BY
	...
	WHEN NOT MATCHED THEN
	INSERT (REFERENCE_PAYMENT, AMOUNT_TRANSACTIONS, AMOUNT_PAYMENT,TOLERANCE, STATUS, MSJ_VALIDATION, REFERENCE_TRANSACTIONS_ID, REFERENCE_TRANSACTIONS, FECHA_CONCILIACION, METODO_RECIBO,FECHA_PAGO)
	VALUES (vc.REFERENCE_PAYMENT,vc.IMPORTE_TMS,null, vc.TOLERANCIA,'P',null,vc.TABLA_PUENTE_IDs,vc.NUMEROS_DOCUMENTOS,sysdate, vc.METODO_RECIBO,VC.FECHA_DEPOSITO);
...
--TO_DATE(sm.fecha_pago,'YYYYMMDD')<TO_DATE('30/09/2021','DD/MM/YYYY')
	...
	WHEN NOT MATCHED THEN
	INSERT (REFERENCE_PAYMENT, AMOUNT_TRANSACTIONS, AMOUNT_PAYMENT,TOLERANCE, STATUS, MSJ_VALIDATION, REFERENCE_TRANSACTIONS_ID, REFERENCE_TRANSACTIONS, FECHA_CONCILIACION, METODO_RECIBO,FECHA_PAGO)
	VALUES (vc.REFERENCE_PAYMENT,vc.IMPORTE_TMS,null, vc.TOLERANCIA,'P',null,vc.TABLA_PUENTE_IDs,vc.NUMEROS_DOCUMENTOS,sysdate, vc.METODO_RECIBO,TO_DATE(vc.Fecha_Pago,'DD/MM/YYYY'));
